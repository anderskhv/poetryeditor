/**
 * Run all form detection tests with dictionary preloaded
 */

import * as fs from 'fs';
import { injectDictionary } from './src/utils/cmuDict';
import { detectPoetricForm } from './src/utils/formDetector';

// Load dictionary
console.log('Loading CMU dictionary...');
const dictText = fs.readFileSync('./public/cmudict.dict', 'utf-8');
...
const testPoems: TestPoem[] = [
  // ===== SHAKESPEARE'S SONNETS =====
  {
    ...
    expectedFit: 'medium', // Has imperfect rhyme
...
// Run tests
console.log('RUNNING FORM DETECTION TESTS');
console.log('='.repeat(80));

let totalTests = 0;
let passedTests = 0;
let failedTests = 0;
const failures: { poem: TestPoem; result: any }[] = [];
...
testPoems.forEach(poem => {
  ...
  const fitMatches = result.fit === poem.expectedFit ||
                    (poem.expectedFit === 'medium' && (result.fit === 'high' || result.fit === 'medium'));

  const passed = formMatches && fitMatches;

  if (passed) {
    passedTests++;
    console.log(`✓ ${poem.title} by ${poem.author}`);
    console.log(`  Form: ${result.form} (${result.fit} fit)`);
...
if (failures.length > 0) {
  ...
  failures.forEach(({ poem, result }, index) => {
    ...
    console.log(`   FitScore: ${result.fitScore || 'N/A'}`);
    if (result.issues) {
      console.log(`   Issues: ${result.issues.join(', ')}`);
    }
  });
